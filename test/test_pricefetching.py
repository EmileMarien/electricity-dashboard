...
class TestPriceFetching(unittest.TestCase):
    def test_fetch_price_xlsx(self):
        print(fetch_electricity_prices_xlsx())
        self.assertEqual(fetch_electricity_prices().shape, (216, 1))
        #check if numeric
        self.assertTrue(fetch_electricity_prices_xlsx()['Belpex'].dtype == 'float64')
        #check if datetimeindex
        self.assertTrue(fetch_electricity_prices_xlsx().index.dtype == pd.DatetimeTZDtype(tz='UTC'))
    ...
    def test_fetch_price(self):
        ...
        #check if datetimeindex
        self.assertTrue(fetch_electricity_prices().index.dtype == pd.DatetimeTZDtype(tz='UTC'))
